...
void DescribePhysicsBindings(LuaApiRegistry& api) {
    ...
    p.Fn("apply_torque(entity, x, y, z)",
         "Twist it about WORLD axes. Continuous, so apply every frame it should act");
    p.Fn("apply_local_torque(entity, x, y, z)",
         "Twist about its OWN axes - roll, pitch and yaw follow the body as it turns");
    p.Fn("apply_angular_impulse(entity, x, y, z)",
         "An instant change of spin, applied once");
    p.Fn("set_angular_velocity(entity, x, y, z)", "Set the spin outright");
    p.Fn("angular_velocity(entity) -> x, y, z",   "Its current spin, as three numbers");
    p.Fn("has_body(entity) -> bool",
