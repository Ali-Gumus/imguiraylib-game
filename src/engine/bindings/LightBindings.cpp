...
void RegisterLightBindings(sol::state& lua) {
    ...
    // Light.setColor(r, g, b): channels are 0..255, matching the numbers the
    // Inspector's colour picker shows, and are converted to the 0..1 the shader
    // works in.
    lt["setColor"] = [findLight](float r, float g, float b) {
        if (auto* l = findLight())
            l->color = {r / 255.0f, g / 255.0f, b / 255.0f};
    };
    // Light.setIntensity(v): brightness, separate from colour. 1 is normal,
    // 0 is night, above 1 is brighter than the surface's own colour.
    lt["setIntensity"] = [findLight](float v) {
        if (auto* l = findLight()) l->intensity = v;
    };
    // Light.setAmbient(r, g, b): the light reaching surfaces the sun cannot
    // see. Lower it for harsh, high-contrast shadows; raise it for a flat,
    // overcast look.
    lt["setAmbient"] = [findLight](float r, float g, float b) {
        if (auto* l = findLight())
            l->ambient = {r / 255.0f, g / 255.0f, b / 255.0f};
    };
    // Light.getIntensity(): read it back, so a script can fade from wherever
    // the light currently is rather than from a number it assumed.
    lt["getIntensity"] = [findLight]() -> float {
        auto* l = findLight();
...
void DescribeLightBindings(LuaApiRegistry& api) {
    ...
    l.Fn("setIntensity(value)", "Set the sun's brightness. Written to the component, not the shader");
    l.Fn("getIntensity() -> number", "Read it back");
    l.Fn("setColor(r, g, b)",
         "Set the sun's colour, channels 0..1. Warms or cools everything the sun touches");
    l.Fn("setAmbient(value)",
